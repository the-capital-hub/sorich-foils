...
const WhySorichFoils: FC = () => {
	...
	}> = ({ feature, index, cardWidthClass }) => (
		<div
			key={index}
			className={`bg-white text-black shadow-md rounded-2xl p-6 ${cardWidthClass} flex flex-col items-center text-center hover:shadow-lg justify-center transition-shadow duration-300 border-b-4 border-black`}
		>
			{feature.icon}
			<h3 className={`text-3xl mb-2 font-medium ${outfit.className}`}>
				{feature.title}
			</h3>
			<p className={`text-gray-600 text-sm font-normal ${outfit.className}`}>
				{feature.desc}
			</p>
		</div>
	);
	...
	return (
		<section className="py-16 bg-[#FEFFFA] text-center text-black">
			<h2 className={`text-3xl lg:text-5xl mb-10 ${instrument.className}`}>
				Why Sorich Foils?
			</h2>
