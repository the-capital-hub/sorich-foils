...
const Navbar = ({ isDark = true }: { isDark?: boolean }) => {
  ...
  const SubNavLinks = [
    ...
    { name: "Milestones", href: "/about/history" },
    { name: "Certificates", href: "certificate" },
  ];
  
 
  const isAboutSection = ["/about", "/infra", "/certificate"].some(prefix =>
  ...
  return (
    ...
    <div className={`absolute top-0 left-0 right-0 w-full z-50 ${ptMono.className}`}>
      ...
      >
        ...
        whileHover={{ y: -2 }}
        whileTap={{ scale: 0.95 }}>
        {pathname === "/" ? <RoundedActionButton /> : <RoundedActionButton 
            text="Contact Us"
      ...
      <AnimatePresence>
        {isAboutSection && (
          <motion.div
            ...
            className="w-full bg-white shadow-lg border-t border-gray-200 lg:block hidden"
          >
                    ...
                    {SubNavLinks.map((link) => (
                        <Link 
                            ...
                            className={`block pl-4 py-2 text-sm ${
                                pathname === link.href ? 'font-bold text-[#9DC834]' : 'text-gray-700 hover:text-gray-900'
                            }`}
                        >
                            {link.name}
                        </Link>
