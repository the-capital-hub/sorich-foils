...
export default function BlogSection() {
  const recentBlog = {
    date: "22 July 2024",
    readTime: "4 min",
    title: "Desiccant Packaging: A Deep Dive into Effective Moisture Control Packaging",
  ...
  const weeklyBlogs = [
    {
      date: "22 July 2024",
      readTime: "4 min",
      title: "Primary Packaging: Mastering Primary Packaging for Impactful Products",
  ...
  return (
    <section className="relative py-20 bg-[#FFFFFF] mt-12">
        <Image
          src={bg}
          alt="Background"
          fill
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="text-center mb-14">
          <h2 className={`text-3xl md:text-7xl font-semibold mb-3 text-[#000000] ${inter.className}`}>
            Catch Up with Our Latest Blogs
          </h2>
          <p className={`text-[#828282] max-w-4xl text-lg mx-auto font-normal lg:mt-8 ${inter.className}`}>
            Catch up with our latest news and stay in the loop on recent updates, insightful
            stories, and exciting announcements shaping our journey forward!
          </p>
        </div>

        {/* Recent Blog */}
        <div className="mb-16 border-t #E0E0E0">
          <h3 className={`text-2xl italic text-[#000000] font-normal mb-4 lg:mt-8 ${inter.className}`}>Recent Blog</h3>
    ...
    />
            ...
            <div className="rounded-2xl bg-[#F9FFEA] p-4 border-b-8 border-r-8 border border-black">
              <p className={`text-lg text-[#4F4F4F] mb-2 ${inter.className} font-semibold`}>
                {recentBlog.date} • {recentBlog.readTime}
              </p>
              <h4 className={`text-3xl font-medium mb-3 ${inter.className} text-[#000000]`}>{recentBlog.title}</h4>
              <p className={`text-[#828282] text-lg mb-4 ${inter.className} font-normal`}>{recentBlog.desc}</p>
        ...
        {/* Weekly Most Read */}
        <h3 className={`text-2xl text-[#000000] font-normal mb-4 ${inter.className}`}>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8 lg:mt-12">
          {weeklyBlogs.map((blog, index) => (
            <div key={index} className="rounded-2xl overflow-hidden hover:shadow-md transition-all">
              <Image
                src={blog.image}
                alt={blog.title}
                width={400}
              ...
              <div className="p-5">
                ...
                <h4 className={`text-2xl text-[#000000] font-regular mb-2 leading-snug ${inter.className}`}>
                  {blog.title}
                </h4>
                <p className={`text-[#828282] text-lg mb-4 ${inter.className}`}>{blog.desc}</p>
                <div className="flex gap-2">
                  {blog.tags.map((tag, i) => (
                    <span
                      key={i}
        ...
        <div className="flex justify-between items-center gap-3 mt-14">
          <button className={`flex items-center gap-1 border border-[#828282] text-black rounded-full px-4 py-2 text-sm hover:shadow-sm ${inter.className} font-normal`}>
