"use client";
import Image from "next/image";
import { ArrowRight } from "lucide-react";
import investors from "@/public/bg2.png";
import { Inter, Manrope } from "next/font/google"; 



const inter = Inter({
    weight: ["700","400"],
    subsets: ["latin"],
    display: "swap",
  });

const manrope = Manrope({
    weight: ["700","400"],
    subsets: ["latin"],
    display: "swap",
  });


const Investors = () => {
  return (
    <section className="min-h-screen bg-white text-gray-900 p-16">
      {/* Hero Section */}
      <div className="flex relative w-[95%] h-[720px] rounded-3xl overflow-hidden mb-16 justify-items-center lg:ml-6">
        <Image
          src={investors}
          alt="Investors"
          
          className="object-cover w-full h-full"
          priority
        />
        <div className="absolute inset-0 bg-black/40 flex flex-col items-center justify-center text-center px-6">
          <h1 className={`text-4xl md:text-6xl font-bold text-white mb-3 ${inter.className}`}>
            Investors
          </h1>
          <p className={`text-white/90 max-w-4xl text-base md:text-xl ${inter.className} font-normal`}>
            Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do
            eiusmod tempor incididunt ut labore et dolore magna aliqua.
          </p>
        </div>
      </div>

      {/* Form Section */}
     

<div className="max-w-7xl mx-auto px-6 pb-20">
  {/* NEW: Top Section Layout (Title/Text + Social Icons) */}
  <div className="flex flex-col md:flex-row justify-between items-start mb-12">
    {/* Left Side: Headings */}
    <div className="max-w-5xl">
      <h3 className={`text-[#000000] mb-3 text-xl uppercase tracking-wide ${manrope.className} font-normal`}>
        Get Started
      </h3>
      <h2 className={`text-3xl md:text-7xl font-bold leading-tight ${manrope.className}`}>
        Information Available on Request
      </h2>
    </div>

    {/* Right Side: Social Icons (Now Static) */}
    <div className="flex md:flex-col gap-3 mt-6 md:mt-8 justify-end max-w-5xl lg:mr-20">
  <a
    href="#"
    className="w-9 h-9 flex items-center justify-center bg-white border rounded-full shadow hover:bg-gray-100 transition"
  >
    <i className="fa-brands fa-facebook-f text-gray-700"></i>
  </a>
  <a
    href="#"
    className="w-9 h-9 flex items-center justify-center bg-white border rounded-full shadow hover:bg-gray-100 transition"
  >
    <i className="fa-brands fa-instagram text-gray-700"></i>
  </a>
  <a
    href="#"
    className="w-9 h-9 flex items-center justify-center bg-white border rounded-full shadow hover:bg-gray-100 transition"
  >
    <i className="fa-brands fa-twitter text-gray-700"></i>
  </a>
</div>
  </div>

  {/* Form Section */}
  <form className="space-y-10">
    {/* Form Row 1 */}
    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
      {/* ... (Your original form inputs) ... */}
      <div className="flex flex-col">
        <label className={`text-xl text-[#000000] mb-1 ${manrope.className} font-normal`}>Your Name</label>
        <input
          type="text"
          placeholder=""
          className="border-b border-gray-300 focus:border-gray-800 outline-none py-2"
        />
      </div>
      <div className="flex flex-col">
        <label className={`text-xl font-normal text-[#000000] mb-1 ${manrope.className}`}>Email Address</label>
        <input
          type="email"
          placeholder=""
          className="border-b border-gray-300 focus:border-gray-800 outline-none py-2"
        />
      </div>
      <div className="flex flex-col">
        <label className={`text-xl font-normal text-[#000000] mb-1 ${manrope.className}`}>
          Phone Number (optional)
        </label>
        <input
          type="tel"
          placeholder=""
          className="border-b border-gray-300 focus:border-gray-800 outline-none py-2"
        />
      </div>
    </div>

    {/* Row 2 */}
    <div>
      <label className={`text-xl font-normal text-[#000000] mb-1 block ${manrope.className}`}>
        Purpose of Request
      </label>
      <input
        type="text"
        className="w-full border-b border-gray-300 focus:border-gray-800 outline-none py-2"
      />
    </div>

    {/* Row 3 */}
    <div>
      <label className={`text-xl font-normal text-[#000000] mb-1 block ${manrope.className}`}>
        Specific Information Required
      </label>
      <textarea
        rows={4}
        className="w-full border-b border-gray-300 focus:border-gray-800 outline-none py-2 resize-none"
      ></textarea>
    </div>

    {/* Submit Button */}
    <div className="pt-4">
      <button
        type="submit"
        className="inline-flex items-center gap-2 px-6 py-2 rounded-full border border-gray-300 text-gray-900 hover:bg-gray-100 transition"
      >
        Send Request
        <span className="w-6 h-6 bg-lime-400 flex items-center justify-center rounded-full">
          <ArrowRight className="w-4 h-4 text-black" />
        </span>
      </button>
    </div>
  </form>
</div>
    </section>
  );
};

export default Investors;
